...
def adjust_volume(data):
    ...
    volume = data['volume_scale']
    volume_db = utils.ratio_to_db(volume)
    
    adjusted_tracks = [track.apply_gain(volume_db) for track in tracks]
    output = sum(adjusted_tracks)
    output.export(data['output'], format="wav")
    print(f"\nAdjusted volume by {volume} and saved to {data['output']}...\n")
    clear(data)
